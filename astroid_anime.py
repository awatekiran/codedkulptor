...
# watch implementation at http://www.codeskulptor.org/#user17_gTaLeKtfCWgk7rp.py
import simplegui
import math
import random
...
def click(pos):
    global started, score,soundtrack, my_ship
...
def process_sprite_group(new_set, canvas):
    p = set(new_set)
    for item in p:
        item.draw(canvas)
        result = item.update()
...
def group_collide(group, other_object):
    ...
    for i in p:
        if i.collide(other_object):
            ...
            explosion = Sprite(i.pos, i.vel, 0, 0, explosion_image, ex_info, explosion_sound)
